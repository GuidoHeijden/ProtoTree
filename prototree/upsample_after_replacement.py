...
def upsample(tree: ProtoTree, project_info: dict, project_loader: DataLoader, folder_name: str, which_nodes: list,
    ...
    with torch.no_grad():
        ...
        for node, j in tree._out_map.items():
            ...
            if node in tree.branches and node.index in which_nodes:  # do not upsample when node is pruned
                ...
                # HERE I ADDED CODE
                try:
                    segvalues, output = segment_image.segmentAsPascalvoc(imgs[prototype_info['input_image_ix']][0],
                                                                         overlay=True)
                except ValueError:
                    print("Something wrong with image segmentation probably... skipping image for now")
                    continue

                output = np.float32(output) / 255
                if output.ndim == 2:  # convert grayscale to RGB
                ...
                overlayed_original_img = 0.5 * output + 0.2 * heatmap
                ...
                masked_similarity_map[
                    prototype_index // W, prototype_index % W] = 1  # mask similarity map such that only the nearest patch z* is visualized
                ...
                high_act_patch = output[high_act_patch_indices[0]:high_act_patch_indices[1],
                ...
                # HERE I ADDED CODE
                x_start, x_end, y_start, y_end = high_act_patch_indices[0], high_act_patch_indices[1], \
                                                 high_act_patch_indices[2], high_act_patch_indices[3]

                ROI = segvalues["masks"][x_start:x_end, y_start:y_end]
                clever_hans_percentage = round(np.sum(ROI) / np.size(ROI), 3)
                log.log_message("Overlap percentage for node "+str(decision_node_idx)+": "+str(clever_hans_percentage))
                upsampling_results.append([decision_node_idx, clever_hans_percentage, clever_hans_percentage > 0.5])

                # save the masked picture with bounding box showing high activation patch
                imsave_with_bbox(
                    ...
                    bbox_width_end=high_act_patch_indices[3], color=(0, 255, 255), text=clever_hans_percentage)

    # I ADDED CODE HERE
    with open(r"upsampling_results.txt", "w", encoding="utf-8") as a_file:
        a_file.write('\n'.join([''.join(str(i)) for i in upsampling_results]))
...
def imsave_with_bbox(fname, img_rgb, bbox_height_start, bbox_height_end,
                     bbox_width_start, bbox_width_end, color=(0, 255, 255), text=None):
    ...
    if text:
        percentage = str(text)
        x_ori = int(bbox_width_start + bbox_width_end / 2)
        y_ori = int(bbox_height_start + bbox_height_end / 2)
        cv2.putText(img_bgr_uint8, percentage, (bbox_width_start, bbox_height_start), cv2.FONT_HERSHEY_SIMPLEX,
                    0.5, (255, 255, 255), 2)
